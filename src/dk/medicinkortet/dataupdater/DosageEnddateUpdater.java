...
public class DosageEnddateUpdater {
	...
	public void update(boolean testMode) {
		...
		String sql = "select ipid.PersonIdentifier, ipid.PersonIdentifierSource, dm.DrugMedicationIdentifier from DrugMedications dm "
				...
				+ " where dm.HashedWithdrawnByPID is null " + " and sd.ValidFrom<now() and sd.ValidTo>now() "
				+ " and sd.IterationInterval=0 " + " and dm.ValidFrom<now() and dm.ValidTo>now() "
		...
		jdbcTemplate.query(sql, rs -> {
			...
			updateDosageEnd(
					...
					rs.getLong("DrugMedicationIdentifier"), testMode);
	...
	public void updateDosageEnd(PersonIdentifierVO personIdentifier, Long drugMedicationIdentifier, boolean testMode) {
		...
		try {
			...
			DrugMedicationOverview drugMedicationOverview = mc.getDrugMedications().stream()
			...
			if (!testMode && updated) {
				...
				ModificatorOtherPersonVO otherPerson = new ModificatorOtherPersonVO((String) null, "DosageEndUpdater",
						null, existingModificator.getOrganisation().getOrganisationName());
				ModificatorVO modifiedBy = ModificatorVO.makeOther(otherPerson, Role.System,
						existingModificator.getOrganisation(), timeService.currentLocalDateTime());
